...
GroupsManagerMenu::GroupsManagerMenu(
    std::shared_ptr<GroupsCollection> groups_collection)
    : Menu() {
  std::vector<std::string> options = {"Create Group", "Enter Group",
                                      "Delete Group", "Exit"};
...
Group::Config GroupsManagerMenu::getGroupConfigFromUser() {
  ...
  uint16_t teams_amount =
      UserInteraction::getUint16FromUser("Amount of teams", 2);
  uint16_t players_in_team_amount =
      UserInteraction::getUint16FromUser("Amount of players in a team", 2);

  Group::Config group_config = {
      static_cast<uint16_t>(m_groups_collection->getSize()), name, teams_amount,
...
bool GroupsManagerMenu::handle() {
  while (!m_exit_menus) {
    ...
    GroupsManagerMenuOption option =
        static_cast<GroupsManagerMenuOption>(getChoiceFromUser());
    switch (option) {
