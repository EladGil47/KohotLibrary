...
GroupMenu::GroupMenu(std::shared_ptr<Group> group) : Menu() {
  std::vector<std::string> options = {"Add Player",   "Delete Player",
                                      "Create Teams", "Change Group Name",
                                      "Go Back",      "Exit"};
...
size_t GroupMenu::getPlayerIndexFromUser() {
  ...
  std::vector<std::string> options =
      m_group->getPlayersCollectionRef().getPlayersNames();
...
void GroupMenu::onDeletePlayer() {
  ...
  if (size > 0) {
    ...
    for (size_t index = player_index; index < size; index++) {
      m_group->getPlayersCollectionRef().getItem(index)->setId(
...
GroupMenu::chooseComingPlayers(uint16_t coming_players_amount) {

  std::vector<std::shared_ptr<Player>> coming_players;

  PlayersCollection players = m_group->getPlayersCollectionRef();
  for (uint16_t chosen_count = 0; chosen_count < coming_players_amount;
       chosen_count++) {
    std::cout << "Creating Teams :) \n\nPlease choose a player from the "
                 "players list\n";
    Menu show_players_menu;
    ...
    show_players_menu.initializeOptions(options);
    size_t index =
        static_cast<size_t>(show_players_menu.getChoiceFromUser() - 1);
...
uint16_t GroupMenu::getTeamsAmountFromUser() {
  ...
  if (teams_amount == 0) {
    const uint16_t MAX_POSSIBLE_CHOICE =
        static_cast<uint16_t>(m_group->getNumOfPlayers()) /
        MIN_TEAM_AMOUNT_CHOICE;
    teams_amount = UserInteraction::getUint16FromUser(
        "Amount of teams", MIN_TEAM_AMOUNT_CHOICE, MAX_POSSIBLE_CHOICE);
  }
  return teams_amount;
}
...
uint16_t GroupMenu::getPlayersInTeamAmountFromUser(uint16_t teams_amount) {
  ...
  if (players_in_team_amount == 0) {
    const uint16_t MAX_POSSIBLE_CHOICE =
        static_cast<uint16_t>(m_group->getNumOfPlayers()) / teams_amount;
    teams_amount = UserInteraction::getUint16FromUser(
        "Amount of players in a team", MIN_PLAYERS_IN_TEAM_AMOUNT,
        MAX_POSSIBLE_CHOICE);
  }
...
bool GroupMenu::handle() {
  while (m_stay_in_menu && !m_exit_menus) {
    ...
    case GroupMenuOption::CREATE_TEAMS: {
      ...
      uint16_t players_in_team_amount =
          getPlayersInTeamAmountFromUser(teams_amount);
      const uint16_t COMING_PLAYERS_AMOUNT =
          teams_amount * players_in_team_amount;
      m_teams =
          TeamsCreator::createTeams(chooseComingPlayers(COMING_PLAYERS_AMOUNT),
                                    teams_amount, players_in_team_amount);
