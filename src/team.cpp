...
void Team::replacePlayer(uint16_t player_index_in_team_list, std::shared_ptr<Player> new_player)
{
    if (m_players_collection)
    {
        m_players_collection->replaceItem(player_index_in_team_list, new_player);
    }
}

void Team::cleanPlayer(uint16_t player_index_in_team_list)
{
    if (m_players_collection)
    {
        m_players_collection->cleanItem(player_index_in_team_list);
    }
}
...
double Team::getAverageRate()
{
    ...
    {
        std::shared_ptr<Player> player = m_players_collection->getItem(player_index);
        rating_sum += (player ? player->getRate() : 0);
    }
